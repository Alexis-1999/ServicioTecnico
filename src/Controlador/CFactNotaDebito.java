...
import Modelo.MFactDebito;
import Vista.VFactNotaDebito;
...
public class CFactNotaDebito implements ActionListener {

     VFactNotaDebito vista;
 MFactDebito modelo = new  MFactDebito();
    ...
    public CFactNotaDebito(VFactNotaDebito vista) {
        this.vista = vista;
    }
    ...
    public void iniciar() {
        ...
        this.vista.ComboFactura.setModel(this.modelo.getListaFacturaCompra());
        deshabilitaBotones();
    }

    public void actionPerformed(ActionEvent e) {
        switch (CFactNotaDebito.AccionMVC.valueOf(e.getActionCommand())) {
            case AGREGAR:
                this.vista.cboInsumos.setEnabled(true);
                this.vista.comboProveedor.setEnabled(true);
                this.vista.ComboFactura.setEnabled(true);
                ...
                this.vista.txtcodfact.setText(this.modelo.NotaDebitoCompra);
                break;
            case GUARDAR:
                 String FacturaCompras[] = this.vista.ComboFactura.getSelectedItem().toString().split("-");
                String proveedor[] = this.vista.comboProveedor.getSelectedItem().toString().split("-");
                int cantidadMercaderias = this.vista.tablaventas.getRowCount();
                JTable tabla3 = this.vista.tablaventas;
                if (this.modelo.RegistrarNotaCompra(
                        this.vista.txtcodfact.getText(),                        
                        this.vista.txtfecha.getText(),
                       
                      FacturaCompras[0].trim(),
                         this.vista.CampoCodUsuario.getText(),
                      proveedor[0].trim()
