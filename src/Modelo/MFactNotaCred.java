...
public class MFactNotaCred extends ConexionBD {

    public ResultSet rs;
   MFactNotaCred vista;
    public int totales3 = 0;
    public String idNotacreditoCompras = "";
 
    public boolean RegistrarNotaCompra(String idNotacreditoCompras, String Fecha, String idFacturaCompras, String idProveedores) {
         String q = " insert into notacreditocompras ( idNotacreditoCompras, Fecha,idFacturaCompras,idProveedores )"
                    + "values( '" + idNotacreditoCompras + "', '" + Fecha + "', '" +idFacturaCompras+ "', '" + idProveedores +"')";
        try {
    ...
    public boolean RegistrarDetalleNotaCompra(String idNotacreditoCompras, String cantidad, String idInsumos, String subtotal ) {
        String q2 = " insert into detallefacturascompra (idNotacreditoCompras, cantidad, idInsumos,subtotal  )"
                    + "values('" + idNotacreditoCompras + "','" + cantidad + "','" + idInsumos +"','" + subtotal + "' )";
        try {
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idNotacreditoCompras) + 1 as nro from notacreditocompras");
            rs = pstm2.executeQuery();
            rs.next();
            if (rs.getString("nro") == null) {
                idNotacreditoCompras = "1";
            } else {
                idNotacreditoCompras = rs.getString("nro");
            }
        } catch (SQLException ex) {
            Logger.getLogger(MFactNotaCred.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
