...
public class MFacturaCompra extends ConexionBD {
    ...
    public String FacturaCompras = "";
 
    public boolean RegistrarCompra(String idFacturaCompras, String Fecha, String idTipofactura, String idProveedores) {
         String q = " insert into facturacompras ( idFacturaCompras, Fecha, idTipofactura,idProveedores )"
                    + "values( '" + idFacturaCompras + "', '" + Fecha + "', '" + idTipofactura+ "', '" + idProveedores +  "')";
        try {
    ...
    public boolean RegistrarDetalleCompra(String FacturaCompras, String cantidad, String precioCompra, String subtotal,String idInsumos ) {
        String q2 = " insert into detallefacturascompra (FacturaCompras, cantidad, precioCompra,subtotal, idInsumos )"
                    + "values('" + FacturaCompras + "','" + cantidad + "','" + precioCompra +"','" + subtotal + "','" + idInsumos +"' )";
        try {
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idFacturaCompras) + 1 as nro from facturacompras");
            rs = pstm2.executeQuery();
            rs.next();
            if (rs.getString("nro") == null) {
                FacturaCompras = "1";
            } else {
                FacturaCompras = rs.getString("nro");
            }
